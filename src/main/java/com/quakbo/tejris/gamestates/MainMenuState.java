...
public class MainMenuState extends BasicGameState {
    ...
    public void pollKeyboard(GameContainer gc, StateBasedGame sbg) {
        ...
        if (input.isKeyPressed(Input.KEY_DOWN)) {
        	...
        	if (cursorPosition.y == 250) {
                sbg.enterState(GameState.GAME_CONFIG_STATE.getId(), new FadeOutTransition(Color.black), new FadeInTransition(Color.black));
            } else if (cursorPosition.y == 325) {
                sbg.enterState(GameState.INSTRUCTIONS_STATE.getId(), new FadeOutTransition(Color.black), new FadeInTransition(Color.black));
